...
// ArtDecal sub-component to handle texture loading with Suspense
...
function CupMesh({ cupModel, art }: CupMeshProps) {
  const { nodes } = useGLTF(cupModel.modelUrl || '/models/cup.glb');
  ...
  if (!modifiedGeometry) {
    console.error("3D Model Error: The 'Cup' mesh or its geometry is missing in the provided GLB file.");
    return null; 
  }
...
export default function CupPreview3D({ cupModel, art, modelUrl }: CupPreview3DProps) {
  ...
  const ErrorFallback = (
    <div className="flex items-center justify-center h-full text-center p-4 bg-card">
        <div className="bg-destructive text-destructive-foreground p-4 rounded-md shadow-lg flex flex-col items-center">
            <PackageX className="w-12 h-12 mb-2" />
            <h3 className="font-bold">Modelo 3D Não Encontrado</h3>
            <p className="text-sm mt-2">
                O arquivo do modelo 3D não foi encontrado ou não pôde ser carregado.
            </p>
        </div>
    </div>
  );
    ...
    <ErrorBoundary fallback={ErrorFallback}>
      <Canvas shadows camera={{ position: [0, 0.2, 3], fov: 50 }}>
        <Suspense fallback={null}>
            ...
            <CupMesh cupModel={{...cupModel, modelUrl: finalModelUrl}} art={art} />
